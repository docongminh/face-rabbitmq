...
int InsightfaceLandmarker::LoadModel(const char * root_path) {
	std::string fl_param = std::string(root_path) + "/landmark/insightface/2d106.param";
	std::string fl_bin = std::string(root_path) + "/landmark/insightface/2d106.bin";
...
int InsightfaceLandmarker::ExtractKeypoints(const cv::Mat & img_src,
	...
	utils::EnlargeRect(enlarge_scale, &face_enlarged);

	// 2 square the rect
	utils::RectifyRect(&face_enlarged);
