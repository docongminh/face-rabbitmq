...
namespace detect {
	...
	int RetinaFace::LoadModel(const char * root_path) {
		std::string fd_param = std::string(root_path) + "/detect/retinaface/retinaface.param";
		std::string fd_bin = std::string(root_path) + "/detect/retinaface/retinaface.bin";
		...
		for (int i = 0; i < 3; ++i) {
			ANCHORS anchors;
			if (0 == i) {
				utils::GenerateAnchors(16, { 1.0f }, { 32, 16 }, &anchors);
			}
			else if (1 == i) {
				utils::GenerateAnchors(16, { 1.0f }, { 8, 4 }, &anchors);
			}
			else {
				utils::GenerateAnchors(16, { 1.0f }, { 2, 1 }, &anchors);
			}
	...
	int RetinaFace::DetectFace(const cv::Mat & img_src,
		...
		std::vector<utils::FaceInfo> faces_tmp;
		for (int i = 0; i < 3; ++i) {
		...
		utils::NMS(faces_tmp, faces, iouThreshold_);
